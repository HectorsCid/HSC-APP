(function(root,factory){const lib=factory();if(typeof module==='object'&&module.exports)module.exports=lib;else root.HscRepairs=lib})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function valveLabel(steps){return steps===0?'Posición original':`${Math.abs(steps)}/8 de vuelta a la ${steps>0?'derecha':'izquierda'} del inicio`}
  ...
  function mount(api){
    ...
    function renderValves(animateId='',fromSteps=0){const valves=editor.data.valve_adjustments||[];$('#repairValves').innerHTML=valves.map((valve,index)=>{const steps=valvePosition(valve.movements);return `<div class="repair-valve" data-valve="${esc(valve.id)}"><div class="repair-valve-head"><b>Válvula ${index+1}</b>${valveDial(steps,valve.id===animateId?fromSteps:steps)}</div><p class="repair-valve-legend"><span class="origin">Inicio</span><span class="current">Posición actual</span></p><label>Identificación (opcional)<input data-valve-name="${esc(valve.id)}" value="${esc(valve.name||'')}" maxlength="100" placeholder="Ej. Circuito 1"></label><p><b>${esc(valveLabel(steps))}</b></p><div class="repair-actions"><button class="secondary" type="button" data-turn="-1" data-valve-id="${esc(valve.id)}">↶ ⅛ izquierda</button><button class="secondary" type="button" data-turn="1" data-valve-id="${esc(valve.id)}">↷ ⅛ derecha</button><button class="secondary" type="button" data-undo="${esc(valve.id)}" ${valve.movements.length?'':'disabled'}>Deshacer último</button></div><p class="repair-valve-steps">${valve.movements.length?valve.movements.map(m=>m.steps>0?'⅛ derecha':'⅛ izquierda').join(' · '):'Sin movimientos todavía'}</p><button class="secondary" type="button" data-remove-valve="${esc(valve.id)}">Quitar válvula</button></div>`}).join('');if(animateId)requestAnimationFrame(()=>{const dial=[...$('#repairValves').querySelectorAll('[data-valve]')].find(el=>el.dataset.valve===animateId)?.querySelector('.repair-valve-moving');if(dial)dial.style.transform=`rotate(${valvePosition(valves.find(v=>v.id===animateId)?.movements)*45}deg)`});$('#repairValves').querySelectorAll('[data-valve-name]').forEach(el=>el.oninput=()=>{valves.find(v=>v.id===el.dataset.valveName).name=el.value;autosave()});$('#repairValves').querySelectorAll('[data-turn]').forEach(el=>el.onclick=()=>{const valve=valves.find(v=>v.id===el.dataset.valveId),from=valvePosition(valve.movements);valve.movements.push({steps:Number(el.dataset.turn),at:new Date().toISOString()});renderValves(valve.id,from);autosave()});$('#repairValves').querySelectorAll('[data-undo]').forEach(el=>el.onclick=()=>{const valve=valves.find(v=>v.id===el.dataset.undo),from=valvePosition(valve.movements);valve.movements.pop();renderValves(valve.id,from);autosave()});$('#repairValves').querySelectorAll('[data-remove-valve]').forEach(el=>el.onclick=()=>{if(!confirm('¿Quitar esta válvula y sus movimientos del borrador?'))return;editor.data.valve_adjustments=valves.filter(v=>v.id!==el.dataset.removeValve);renderValves();autosave()})}
    $('#repairAddValve').onclick=()=>{editor.data.valve_adjustments ||= [];editor.data.valve_adjustments.push({id:uid(),name:'',movements:[]});renderValves();autosave()};
  ...
  return {mount,visible,label,deletableDraft,transmit,valvePosition,valveLabel};
});
