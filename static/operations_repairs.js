(function(root,factory){const lib=factory();if(typeof module==='object'&&module.exports)module.exports=lib;else root.HscRepairs=lib})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function mount(api){
    ...
    function fill(){fields.forEach(k=>form.elements[k].value=editor.data[k]||'');$('#repairClient').value=editor.data.client_id?'client:'+editor.data.client_id:editor.data.client_key;$('#repairEquipment').value=editor.data.equipment_id?'equipment:'+editor.data.equipment_id:editor.data.equipment_key;const persisted=(editor.data.revision||0)>0;$('#repairClient').disabled=persisted;$('#repairEquipment').disabled=persisted;form.elements.client_name.readOnly=!!editor.data.client_id;form.elements.equipment_name.readOnly=!!editor.data.equipment_id;$('#repairClientNameField').hidden=!!(editor.data.client_id&&editor.data.client_name);$('#repairEquipmentNameField').hidden=!!(editor.data.equipment_id&&editor.data.equipment_name);$('#repairDiscardDraft').hidden=!deletableDraft(editor);renderPhotos()}
  ...
  return {mount,visible,label,deletableDraft,transmit};
});
