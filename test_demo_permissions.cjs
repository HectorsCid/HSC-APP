...
assert.match(html, /function canonicalPath\(view\)/);
assert.match(html, /function parentView\(view\)/);
assert.match(html, /La ruta[\s\S]*se calcula por jerarquía/);
...
assert.doesNotMatch(html, /data-switch-mode=/);
assert.doesNotMatch(html, /Activar administrador|Usar como técnico/);
...
assert.match(html, /data-default-closed/);
assert.match(html, /edit_report_id/);
assert.match(html, /reportSevereFault/);
assert.match(html, /reportPartsList/);
assert.match(html, /\/api\/operaciones\/expenses\/\$\{encodeURIComponent\(selectedExpense\)\}/);
...
assert.match(script, /await queueReportOnDevice\(upload\)/);
assert.match(script, /showView\('equipment',false\);[\s\S]*finishReportInBackground\(upload\)/);
...
assert.match(html, /id="profileThemeToggle"/);
assert.match(html, /id="offlineSyncMode"/);
assert.match(html, /id="syncPendingNow"/);
assert.match(script, /function shouldSyncNow\(manual=false\)/);
assert.match(script, /Reporte guardado en este dispositivo/);
assert.match(script, /if\(!upload\.id\).*\/api\/operaciones\/reports\/draft/);
assert.match(html, /id="openExpenses"/);
assert.match(html, /id="openUsersProfile"/);
...
assert.match(html, /data-expense-status="Aprobado"/);
assert.match(script, /\/api\/operaciones\/expenses/);
assert.match(html, /data-style-option="corporate"/);
assert.match(html, /data-style-option="fusion"/);
assert.match(html, /data-style-option="glass"/);
assert.match(html, /data-style-option="classic"/);
assert.match(script, /localStorage\.setItem\('hsc-style',style\)/);
...
assert.doesNotMatch(html, /id="clientsModeBar"/);
assert.match(html, /id="matrixStatus" hidden/);
...
assert.match(html, /id="partnerStyleAction"/);
assert.match(html, /data-view="partnerDocuments"/);
assert.match(html, /data-partner-action="documents"/);
assert.match(html, /data-partner-doc-tab="quotes"/);
assert.match(html, /id="partnerDocumentsList"/);
assert.match(script, /\/api\/operaciones\/partner-documents/);
assert.match(script, /function renderPartnerDocuments\(\)/);
assert.match(html, /id="partnerAppearanceModal"/);
assert.match(html, /id="partnerThemeToggle"/);
...
assert.match(script, /\/api\/operaciones\/faults\/\$\{encodeURIComponent\(fault\.id\)\}\/resolve/);
...
console.log('OK: propietario unificado, IDs protegidos, permisos por técnico y regreso interno presentes.');
